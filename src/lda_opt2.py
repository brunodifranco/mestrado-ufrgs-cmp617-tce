...
class LDAOptimization:
    ...
    def bayesian_opt_objective(
        ...
        returning the Coherence score c-v, c-uci, and c-npmi.
        ...
        -------
        Tuple[float, float]
            Coherence scores u_mass and uci of the LDA model.
        """
        ...
        # Coherence Scores
        coherence_model_u_mass = CoherenceModel(
            model=lda_model, texts=vec, dictionary=id2word, coherence="u_mass", topn=5
        )
        coherence_u_mass = coherence_model_u_mass.get_coherence()

        coherence_model_uci = CoherenceModel(
            model=lda_model, texts=vec, dictionary=id2word, coherence="c_uci", topn=5
        )
        coherence_uci = coherence_model_uci.get_coherence()

        return coherence_u_mass, coherence_uci
    ...
    def get_opt(
        ...
        trial = study.best_trials[0]  # Gets the best trial

        results = {}
        results["filter"] = self.n_filter
        results["nlp_normalization_method"] = self.nlp_normalization_method
        results["best_score_u_mass"] = trial.values[0]  # Coherence u_mass
        results["best_score_uci"] = trial.values[1]  # Coherence c-uci
    ...
    def save_results(
        ...
        output_path = f"src/opt_outputs_2/results_{self.nlp_normalization_method}_with_filter_{self.n_filter}.json"
...
if __name__ == "__main__":
    optimizer = LDAOptimization(
        ...
        n_filter=500,  # Minimum frequency to retain a token in the dictionary. e.g if n_filter=0 all tokens will be kept
